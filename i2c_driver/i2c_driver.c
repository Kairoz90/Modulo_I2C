/*
 * Nombre de Archivo: i2c.c
 *
 * Descripción: Driver I2C
 *
...
static void i2c_interrup_handler_default(void *arg);
static void IRAM_ATTR i2c_master_inicio_cmd_static(i2c_port_t i2c_num, portBASE_TYPE* HPTaskAwoken);
static esp_err_t i2c_hardware_fsm_reset(i2c_port_t i2c_num);
...
void lcd_init (void)
{
	// Inicializacion de 4 bit
	usleep(50000);  		// Espera >40ms
	lcd_send_cmd (0x30);
	usleep(5000);  			// Espera >4.1ms
	lcd_send_cmd (0x30);
	usleep(200);  			// Espera >100us
	lcd_send_cmd (0x30);
	usleep(10000);
  ...
  // Inicializacion Display
	lcd_send_cmd (0x28); 	// Set de Funciones --> DL=0 (Modo 4 bit), N = 1 (Display de 2 lineas) F = 0 (5x8 caracteres de la matriz)
	usleep(1000);
	lcd_send_cmd (0x08);	// Control de Display On/Off  --> D=0,C=0, B=0  ---> Display Off
	usleep(1000);
	lcd_send_cmd (0x01);  	// Limpia el Display
	usleep(1000);
	usleep(1000);
	lcd_send_cmd (0x06); 	//Entra a Set Mode --> I/D = 1 (Incrementa cursor) & S = 0 (Sin cambios)
	usleep(1000);
	lcd_send_cmd (0x0C); 	//Control Display on/off --> D = 1, C y B = 0. (Cursor y blink, Ultimos 2 bits)
	usleep(1000);
}
...
// Set Parametros Configuracion LCD I2C
esp_err_t set_i2c_lcd(void)
{
    i2c_config_t i2c_config_lcd = {};

    i2c_config_lcd.mode = I2C_MODE_MASTER;
    i2c_config_lcd.sda_io_num = GPIO_NUM_16;
    i2c_config_lcd.scl_io_num = GPIO_NUM_17;
    i2c_config_lcd.sda_pullup_en = GPIO_PULLUP_ENABLE;
    i2c_config_lcd.scl_pullup_en = GPIO_PULLUP_ENABLE;
    i2c_config_lcd.master.clk_speed = 100000;

    i2c_configuracion_parametros(I2C_NUM_1, &i2c_config_lcd);

    return i2c_instalacion_driver(I2C_NUM_1, I2C_MODE_MASTER, 0, 0, 0);
}

// Set Parametros Configuracion ESP32 Slave
esp_err_t set_i2c(bool MS_SELECT)
{
	...
	if (!MS_SELECT)
	{
		...
		ESP_ERROR_CHECK(i2c_configuracion_parametros(I2C_NUM_0,&i2c_config));
		ESP_ERROR_CHECK(i2c_instalacion_driver(I2C_NUM_0,I2C_MODE_MASTER,0,0,ESP_INTR_FLAG_LEVEL1));
	}
	else
	{
	    ...
	    i2c_config.slave.slave_addr = I2C_SLAVE_ADDR1;
		ESP_ERROR_CHECK(i2c_configuracion_parametros(I2C_NUM_0,&i2c_config));
		ESP_ERROR_CHECK(i2c_instalacion_driver(I2C_NUM_0,I2C_MODE_SLAVE,RX_BUFFER_LEN,TX_BUFFER_LEN,ESP_INTR_FLAG_LEVEL1));
	}

	return ESP_OK;
}

static void i2c_hardware_disable(i2c_port_t i2c_num)
{
...
static void i2c_hardware_enable(i2c_port_t i2c_num)
{
    I2C_ENTER_CRITICAL(&(i2c_context[i2c_num].spinlock));
...
has only 32 bytes.
*/
esp_err_t i2c_instalacion_driver(i2c_port_t i2c_num, i2c_mode_t mode, size_t slv_rx_buf_len, size_t slv_tx_buf_len,
                             int intr_alloc_flags)
{
    ...
    //hook isr handler
    i2c_interrup_register(i2c_num, i2c_interrup_handler_default, p_i2c_obj[i2c_num], intr_alloc_flags, &p_i2c_obj[i2c_num]->intr_handle);
...
static void IRAM_ATTR i2c_interrup_handler_default(void *arg)
{
    ...
    if (p_i2c->mode == I2C_MODE_MASTER) {
        ...
        if (evt_type == I2C_INTR_EVENT_NACK) {
            ...
            i2c_master_inicio_cmd_static(i2c_num, &HPTaskAwokenCallee);
 ...
 **/
static esp_err_t i2c_master_limpia_bus(i2c_port_t i2c_num)
{
#if !SOC_I2C_SUPPORT_HW_CLR_BUS
    ...
    i2c_gpio_pin_config(i2c_num, sda_io, scl_io, 1, 1, I2C_MODE_MASTER);
#else
...
// hardware reset.
    ...
    i2c_hardware_disable(i2c_num);
    i2c_master_limpia_bus(i2c_num);
    i2c_hardware_enable(i2c_num);
...
static i2c_sclk_t i2c_get_clock_src(const uint32_t clk_flags, const uint32_t clk_speed)
{
...
esp_err_t i2c_configuracion_parametros(i2c_port_t i2c_num, const i2c_config_t *i2c_conf)
{
    ...
    if (i2c_conf->mode == I2C_MODE_MASTER) {
        src_clk = i2c_get_clock_src(i2c_conf->clk_flags, i2c_conf->master.clk_speed);
...
#else
        src_clk = i2c_get_clock_src(i2c_conf->clk_flags, i2c_conf->slave.maximum_speed);
        ESP_RETURN_ON_FALSE(src_clk != I2C_SCLK_MAX, ESP_ERR_INVALID_ARG, I2C_TAG, I2C_CLK_FLAG_ERR_STR);
#endif
    }

    ret = i2c_gpio_pin_config(i2c_num, i2c_conf->sda_io_num, i2c_conf->scl_io_num,
...
esp_err_t i2c_interrup_register(i2c_port_t i2c_num, void (*fn)(void *), void *arg, int intr_alloc_flags, intr_handle_t *handle)
{
    ESP_RETURN_ON_FALSE(i2c_num >= 0 && i2c_num < I2C_NUM_MAX, ESP_ERR_INVALID_ARG, I2C_TAG, I2C_NUM_ERROR_STR);
...
esp_err_t i2c_gpio_pin_config(i2c_port_t i2c_num, int sda_io_num, int scl_io_num, bool sda_pullup_en, bool scl_pullup_en, i2c_mode_t mode)
{
...
static inline bool i2c_command_link_is_static(i2c_cmd_desc_t *cmd_desc)
{
...
void i2c_cmd_link_delete_static(i2c_cmd_handle_t cmd_handle)
{
    ...
    if (cmd == NULL || !i2c_command_link_is_static(cmd)) {
        return;
    }
...
static esp_err_t i2c_command_allocate(i2c_cmd_desc_t *cmd_desc, size_t n, size_t size, void** outptr)
{
    esp_err_t err = ESP_OK;

    if (i2c_command_link_is_static(cmd_desc)) {
...
static inline void i2c_command_log_alloc_error(i2c_cmd_desc_t *cmd_desc)
{
    if (i2c_command_link_is_static(cmd_desc)) {
...
static esp_err_t i2c_command_link_append(i2c_cmd_handle_t cmd_handle, i2c_cmd_t *cmd)
{
    esp_err_t err = ESP_OK;
    ...
    if (cmd_desc->head == NULL) {
        err = i2c_command_allocate(cmd_desc, 1, sizeof(i2c_cmd_link_t), (void**) &cmd_desc->head);
        if (err != ESP_OK) {
            i2c_command_log_alloc_error(cmd_desc);
            return err;
        }
        ...
        err = i2c_command_allocate(cmd_desc, 1, sizeof(i2c_cmd_link_t), (void**) &cmd_desc->cur->next);
        if (err != ESP_OK) {
            i2c_command_log_alloc_error(cmd_desc);
            return err;
        }
...
esp_err_t i2c_master_start(i2c_cmd_handle_t cmd_handle)
{
    ...
    return i2c_command_link_append(cmd_handle, &cmd);
}
...
static esp_err_t i2c_master_lee_static(i2c_cmd_handle_t cmd_handle, uint8_t *data, size_t data_len, i2c_ack_type_t ack)
{
    i2c_cmd_t cmd = {
        .hw_cmd = {
            .ack_val = ack & 0x1,
...
esp_err_t i2c_master_read(i2c_cmd_handle_t cmd_handle, uint8_t *data, size_t data_len, i2c_ack_type_t ack)
{
    ...
    if (ack != I2C_MASTER_LAST_NACK) {
        ret = i2c_master_lee_static(cmd_handle, data, data_len, ack);
    } else if (data_len == 1) {
        ret = i2c_master_lee_byte(cmd_handle, data, I2C_MASTER_NACK);
    } else {
        /* In this case, we have to read data_len-1 bytes sending an ACK at the end
         * of each one.
         */
        ret = i2c_master_lee_static(cmd_handle, data, data_len - 1, I2C_MASTER_ACK);

        /* Last byte has to be NACKed. */
        if (ret == ESP_OK) {
            ret = i2c_master_lee_byte(cmd_handle, data + data_len - 1, I2C_MASTER_NACK);
        }
    }
...
static inline bool i2c_command_un_solo_byte(const i2c_cmd_t *cmd) {
...
static void IRAM_ATTR i2c_master_inicio_cmd_static(i2c_port_t i2c_num, portBASE_TYPE* HPTaskAwoken)
{
    ...
    while (p_i2c->cmd_link.head) {
        ...
        if (cmd->hw_cmd.op_code == I2C_LL_CMD_WRITE) {
            ...
            if (!i2c_command_un_solo_byte(cmd)) {
            ...
            p_i2c->cmd_idx = 0;
            if (i2c_command_un_solo_byte(cmd) || cmd->total_bytes == cmd->bytes_used) {
                p_i2c->cmd_link.head = p_i2c->cmd_link.head->next;
...
static bool is_command_link_buffer_interno(const i2c_cmd_link_t *link)
{
         ...
         cmd_link = cmd_link->next)
    {
        ...
        const bool data_pointer = (cmd_link->cmd.hw_cmd.op_code == I2C_LL_CMD_WRITE && !i2c_command_un_solo_byte(&cmd_link->cmd))
...
esp_err_t i2c_master_cmd_begin(i2c_port_t i2c_num, i2c_cmd_handle_t cmd_handle, TickType_t ticks_to_wait)
{
    ...
    if ( (p_i2c_obj[i2c_num]->intr_alloc_flags & ESP_INTR_FLAG_IRAM) &&
         !is_command_link_buffer_interno(((const i2c_cmd_desc_t *)cmd_handle)->head) ) {
    ...
    if (p_i2c->status == I2C_STATUS_TIMEOUT
        ...
        i2c_hardware_fsm_reset(i2c_num);
        clear_bus_cnt = 0;
    }
    i2c_reset_tx_queue(i2c_num);
    i2c_reset_rx_queue(i2c_num);
     ...
     * `i2c_master_inicio_cmd_static` will also reset this field for each node
    ...
    i2c_master_inicio_cmd_static(i2c_num, NULL);
