...
void lcd_send_cmd (char cmd);  			// Manda command a la LCD

void lcd_send_data (char data);  		// Manda data a la LCD

void lcd_send_string (char *str);  		// Manda string a la LCD

void lcd_put_cur(int row, int col);  	// Pone el cursor en la posicion elegida fila [row] (0 o 1)
										// y en la columna deseada [col] (0-15)

void lcd_clear (void);

esp_err_t set_i2c_lcd(void);
/*
	* @brief Configura el modulo I2C de la LCD
	*
	* @return
	*     - ESP_OK   				-> Correcto.
	*/
...
/*
	* @brief Configura el modulo I2C Slave Master de ESP32
	*
	* @param MS_SELECT			-> Seleccciona entre Master o Slave
	* @return
	*     - ESP_OK   				-> Correcto.
	*/
...
/**
 * @brief Realiza una lectura a un dispositivo conectado a un puerto I2C concreto.
 *        Esta función contiene a `i2c_master_start()`, `i2c_master_write()`, `i2c_master_read()`, etc.
 *        Sólo se llamará en modo maestro I2C.
 *
 ...
 * @param read_buffer 			-> Buffer para almacenar los bytes recibidos en el bus
 * @param read_size 			-> Tamaño, en bytes, del búfer de lectura
 * @param ticks_to_wait 		-> Ticks máximos a esperar antes de emitir un timeout.
 *
 * @return
 *     - ESP_OK   				-> Correcto.
 *     - ESP_ERR_INVALID_ARG 	-> Error de parámetro.
 *     - ESP_FAIL 				-> Error de comando de envío, el esclavo no ha reconocido la transferencia (ACK).
